...
## D4-13. 네트워크 인프라 보안 통제

### 방화벽 vs 네트워크 장비 구분
```
Firewall = 네트워크 세그먼트 간 비인가 트래픽 차단 (1차 보안 도구)
Router   = 패킷 필터링 가능하나 보안 목적 아님 (라우팅이 본업)
L2 Switch = MAC 기반 트래픽 분리 (인가/비인가 구분 불가)
VLAN     = 포트 간 트래픽 통제 (논리적 분리만)
...
물리 보안·부트 암호 = 논리적 취약점 미해결 (별개 문제)
로깅 = 탐지 통제 (Hardening은 예방 통제)
```

### HW 유지보수 프로그램 감사
```
검증 기준 = 벤더 사양(Vendor Specifications) 대비
  - 비계획 유지보수는 예약 불가
  - 이력 추세 분석·운영위원회 승인 불필요
  - 독립적 검증(Independent Verification)이 핵심
```

### 네트워크 관리 핵심 구성요소
```
가장 중요 = Configuration & Change Management
  - 기본 비밀번호 제거
  - 불필요 서비스 비활성화

토폴로지 매핑 = SPOF/격리 식별 (2순위)
모니터링, 프록시 = 부분적 역할
```

### Web Services / OS 패치
```
Web Services 최대 장점 = 효율적 인터페이싱(Interoperability)
  OS·프로그래밍 언어 무관하게 시스템 간 정보 교환 (보안·성능·문서화 향상 아님)

OS 패치 운영 적용 최우선 = 정보 자산 소유자(Information Asset Owner) 승인
  비즈니스 중단 방지 (벤더 테스트 전제, 보안 담당자 매번 승인 불필요)

OS 보안 구성 감사 = 파라미터 설정(Parameter Settings) 검토
  시스템 실행 방식 결정 (트랜잭션 로그=앱 수준, 인가 테이블=논리 접근, 라우팅 테이블=OS 무관)
```

### 네트워크 진단 도구
```
Protocol Analyzer (Wireshark) = 패킷 캡처·모니터링·기록 (진단)
Online Monitor = 전송 정확성·완전성
Downtime Report = 회선 가용성 추적 (보고서)
...
### 헬프데스크 인시던트 종료
```
MOST concern = 최종 사용자 확인 없이 인시던트 종료
  - 서비스 지향 조직 → 사용자 통보 후 종결 필수
  - 미해결 콜 = 에스컬레이션으로 처리
  - 전용 회선·메신저 부재 = 부차적
```
...
```
비인가 SW 의심 시:
  FIRST = 테스트로 사용 여부 검증 (경영진 진술만으로 불충분)
  → 충분한 증거 확보 후 보고서 포함 (독립적 검증 원칙)

비인가 SW 발견 시:
  = 사용 보고 + 재발 방지 필요성 권고
  = 감사인 직접 삭제 X (집행자 역할 X)
```

### 오픈소스 SW 라이선스 리스크
```
GREATEST concern = 라이선스 조건 준수(License Compliance)
  Viral License(GPL 등) = 완성 제품도 동일 권리 부여 의무
  → 지적재산권·법적 리스크
...
### 데스크톱 SW 준수 검토
```
GREATEST concern = 승인 표준 목록에 없는 SW 설치 (정책 위반)
  - 보안·법적·재정 리스크
  - 미문서화·미교육·라이선스 만료는 하위
```

### 보안 정책과 기술 통제 순서
```
비인가 SW 관련 정책 없음 → FIRST = 보안 정책 업데이트 권고
  정책 = 기술 통제의 권한 근거
  정책 → 기술 통제 → 예외 절차 (이 순서!)

IT 부서 통제 구현은 정책 수립 후
다운로드만 차단 → CD/USB는 미해결
```

---

## D4-15. 작업 스케줄링 / 코드 무결성

### 작업 스케줄링 오버라이드
```
운영자가 작업을 오버라이드 = GREATEST concern
  - 비인가 데이터·프로그램 변경 가능
  - 긴급 변경 = 문서화 시 허용
  - 미완료 작업 = 조사 대상
  - 스케줄만 실행 = 정상
...
### UAT 후 운영 코드 변조 방지
```
UAT 통과 후 운영 코드 변조 → 완화 통제 = 직무 분리(SoD)
  - 개발자는 개발 환경만 접근, 운영 접근 차단
  - 회귀 테스트·감사 추적·접근 검토 = 근본 원인 해결 아님
```

### 보안 패치 검증
```
운영 서버 패치 검증 BEST = 자동화 도구(Automated Tool)
  - 즉시 패치 적용 여부 확인 + 누락 보고서 제공
...
### 데이터 처리 오류 탐지
```
데이터 무결성 오류 탐지 = Hash Totals
  - Edit Check, 입력 화면, SoD = 예방 통제
  - Hash Totals = 탐지 통제

"탐지"와 "예방"을 구분해서 답을 고를 것!
```

---

## D4-16. 데이터 전송 / 분산 시스템 / 클라우드

### 수동 암호화 리스크
```
FTP 수동 암호화 GREATEST concern = 사용자가 암호화 잊을 수 있음
  - 미암호화 전송 시 민감 데이터 노출
  - 자격 증명 이메일 전송 = 암호화 시 도난해도 읽기 불가
  - 공유 ID = 책임 추적보다 데이터 보호가 우선
```

### API = 분산 시스템/클라우드 통신
```
분산 시스템 컴포넌트 간 통신·조정 = API (가장 포괄적)
  - 규칙·프로토콜 정의로 데이터 교환
  
비교:
  Message Queue = 비동기 메시지 (특정 방식, 범용 X)
  RPC = 원격 프로시저 호출 (조정 특화 X)
  통신 인프라 = 플랫폼↔인프라 인터페이스
...
## D4-17. Shadow IT / 최종 사용자 컴퓨팅

### 업무 시간 네트워크 성능 저하
```
원인 = 비인가 네트워크 활동(Unauthorized Activities)
  - 파일 공유·도박·대용량 개인 이메일
  - "업무 시간에만" = 직원 활동 패턴과 일치

오답:
  - 맬웨어·방화벽·스팸 = 업무 시간에만 제한되지 않음
```

### 개인 저장 장치 연결 리스크
```
USB, 디지털 액자 등 연결 시 PRIMARY risk = 맬웨어 감염
  - 공장 제조 시 이미 감염 가능
  - 기업 전체에 광범위·심각 피해
  - 데이터 유출·드라이버 충돌·부적절 콘텐츠는 하위

EUC(최종 사용자 컴퓨팅) 주요 리스크:
  = 테스트·IT 일반 통제 미적용
  = 독립 검토 부재, 정식 개발 방법론 미사용
```

---

## D4-18. 용량 모니터링 / IT 자원 관리

### IT 자원 지속적 모니터링 초점
```
최우선 = 정확한 가용성/용량 피드백 (현재 상태 정확성)
  SLA 충족 확인·계획 데이터·예측보다 정확한 현황 파악이 핵심
  시스템 오프라인이어도 SLA 충족 가능

기술 용량 모니터링 주 목적 = SLA(서비스 수준) 충족 보장
  HW 조달·미래 예측·최적 운영은 부차적
  IT의 1차 의무 = 비즈니스 서비스 수준 요구 충족
```

### SaaS SLA 모니터링 / 성능 우려 시 대응
```
SaaS SLA 가용성 모니터링 최선 = 자체 폴링 도구(내부 모니터링)
  벤더 보고서만으로 부족(평가받는 당사자 자료), 사용자 보고는 누락 가능, 제3자는 비용 비효율적

성능 우려 시 최우선 = Baseline 개발 + 모니터링
  실증 데이터 확보 후 판단 (즉시 변경 X, 대체 절차 X, 매뉴얼 X)

웹 애플리케이션 무중단 접근 보장 = 로드 밸런싱
  다중 서버 트래픽 분산 + 서버 장애 시 자동 리디렉션 (미러링·RAID=디스크만, DNS=가용성 무관)
```
