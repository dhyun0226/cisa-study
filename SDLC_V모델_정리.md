...
> 매일 보는 치트시트: `감사인_마인드셋_치트시트.md` (별도 파일)
...
### 시험 전략 (★ 매일 보기)
- [S1. CISA 자주 나오는 패턴 10가지](#s1-cisa-자주-나오는-패턴-10가지-) ★
- [S2. Prevent vs Detect 통제 구분](#s2-prevent-vs-detect-통제-구분-) ★★
- [S3. 무단 변경 3가지 패턴](#s3-무단-변경-관련-3가지-패턴-) ★
- [S4. 한글 번역 함정 + 감사인 역할](#s4-한글-번역-함정--감사인-역할-구분)
- [S5. GREATEST concern 풀이법 (CIA + 조직 맥락)](#s5-greatesthighest-풀이법-cia--조직-맥락)
- [S6. 감사 테스트 순서: 준수 → 실증](#s6-감사-테스트-순서-준수--실증)
- [S6-1. 통제 4유형](#s6-1-통제-4유형-)
- [S7. "FIRST" 문제 풀이법](#s7-first-문제-풀이법-)

### Domain 3 — 개발/구현
- [D3-1. SDLC와 V모델](#d3-1-sdlc와-v모델)
- [D3-2. 테스트 (유형/감사기법/문서)](#d3-2-테스트-유형감사기법문서)
- [D3-3. 프로젝트 관리 (도구/역할/문서)](#d3-3-프로젝트-관리-도구역할문서)
- [D3-4. QC vs QA / 품질 / CMM](#d3-4-qc-vs-qa--품질--cmm)
- [D3-5. 입출력 통제와 시스템 통합](#d3-5-입출력-통제와-시스템-통합)
- [D3-6. 형상관리와 환경 분리](#d3-6-형상관리와-환경-분리)
- [D3-7. 기타 (타당성/PIR/Agile/CI-CD/EUC)](#d3-7-기타-타당성pir애자일cicdeuc)

### Domain 4 — 운영/복구
- [D4-1. 변경/패치/형상 관리 종합](#d4-1-변경패치형상-관리-종합)
- [D4-2. 직무분리와 특권 계정 / Accountability](#d4-2-직무분리와-특권-계정--accountability)
- [D4-3. 재해복구: 사이트/RTO/RPO/MTO](#d4-3-재해복구-사이트rtorpomto)
- [D4-4. 이중화 (Redundancy) 기술 매트릭스](#d4-4-이중화-redundancy-기술-매트릭스)
- [D4-5. 용량/성능/모니터링/시설](#d4-5-용량성능모니터링시설)
- [D4-6. 로그/보고서 종합](#d4-6-로그보고서-종합)
- [D4-7. 근본 원인 분석 (RCA)](#d4-7-근본-원인-분석-rca)
- [D4-8. SLM / SLA / 외주 / BCP / BIA](#d4-8-slm--sla--외주--bcp--bia)
- [D4-9. 데이터베이스 관리](#d4-9-데이터베이스-관리)
- [D4-10. OLTP / OLAP / DW / ETL](#d4-10-oltp--olap--dw--etl)
- [D4-11. 기타 운영 통제 모음](#d4-11-기타-운영-통제-모음)
- [D4-12. 백업 vs 복제 vs 가용성](#d4-12-백업-vs-복제-vs-가용성-)

### Domain 5 — 보안/네트워크
- [D5-1. 네트워크 보안](#d5-1-네트워크-보안)
- [D5-2. 서버/OS Hardening](#d5-2-서버os-hardening)
- [D5-3. API / 외부 저장장치 / SCADA](#d5-3-api--외부-저장장치--scada)
- [D5-4. 정책 우선 / 공모](#d5-4-정책-우선--공모)
...
문제 발견 → 바로 결론/조치 ❌
문제 발견 → 원인 파악 → 권고 ✅
예: 프로젝트 초과 → "관리 실패" ❌ → 비즈니스 케이스/PM 검토 먼저 ✅
예: 인증 느림 → 대역폭 증설 ❌ → 인증 서버 측정 기준 수립 ✅
```

### 패턴 9: 예방 > 탐지
```
입력 오류 → 입력 시점 차단(Limit Check) > 출력 후 재계산
단, 문제가 "detect"라고 묻는 경우엔 탐지 통제가 정답 (S2 참조)
```

### 패턴 10: 크고 복잡한 시스템 → 단계별 배포
```
대규모 배포 = Phased Approach
한 번에 배포(빅뱅) = 리스크 높음
```
...
> 통제 4유형(Preventive/Detective/Corrective/Compensating) 전체 표는 [S6-1](#s6-1-통제-4유형-) 참고.

### 키워드로 통제 유형 식별

| 한글 키워드 | 영어 | 묻는 통제 유형 |
|---|---|---|
| 막다, 방지, **예방** | prevent | 예방 통제 |
| **탐지**, 발견, 식별 | detect, identify | 탐지 통제 |
| **탐지되지 않은** | undetected | 탐지 통제 |
| 무단의, 승인되지 않은 | unauthorized | 보통 둘 다 가능 (동사 확인) |
| 복구 | recover | 복구 통제 |

### 같은 사건, 다른 답
```
"무단 변경을 prevent하려면?" → 접근 제한, 직무분리, 승인
"무단 변경을 detect하려면?"  → 로그 검토, 해시 비교, 모니터링
...
### 풀이 공식
```
1단계: 동사/형용사 키워드 찾기 ("탐지되지 않은" → 탐지 문제)
2단계: 선택지를 통제 유형별 분류
3단계: 묻는 유형만 후보로 남기고 나머지 탈락
4단계: 후보 중 가장 효과적인 것 선택
```

### 로그 함정
```
"로그가 있다(logged)"     = 통제 아님
"로그를 검토한다(reviewed)" = 통제
선택지에 logged만 있고 reviewed 없으면 → 의심
```
...
| 문제 키워드 | 묻는 것 | 정답 |
|---|---|---|
| 있는지 확인 / 일관되게 적용 | 평가 활동 | **준수 테스트** |
| 탐지(detect) / undetected | 변경 사실 발견 | **해시 키 비교** |
| 추적(trace) / 무단 변경 BEST | 사후 역추적 | **실제 변경 → 승인 기록 역추적** |

### 셋의 차이
```
1. 준수 테스트: "변경 관리 통제가 잘 지켜지나?" → 샘플 → 절차/승인과 대조
2. 해시 비교:  "파일이 바뀌었나?"          → 현재 해시 vs 승인 해시
3. 역추적:     "이 변경에 근거가 있나?"     → 실제 변경에서 출발 → 승인 기록 추적
```

### 추적 방향이 핵심
```
순방향: 티켓/문서 → 프로그램
  = 기록에 있는 것만 봄 → 무단 변경 못 잡음 ❌

역방향: 프로그램/실제 변경 → 티켓/승인
  = 모든 변경을 봄 → 무단 변경 발견 ✅

감사의 기본: "문서가 완벽하다고 가정하지 마라"
  → 실제 상태에서 출발 → 문서와 대조
```
...
```
공장 부품이 바뀌었는지 확인:
1. 준수 테스트 = "품질 검사 절차를 잘 지켰나?" 평가
2. 해시 비교  = 부품 봉인 스티커 → 찢어지면 바뀐 것
3. 역추적     = 현재 부품 → "이거 바꾼 지시서 있어?" 확인
```

### 함정
```
"무단 변경 확인" → 시스템 로그 분석 ❌ / 준수 테스트 ✅
로그 분석 = 데이터 수집(재료), 준수 테스트 = 평가(요리)
로그만 봐서는 "승인됐는지" 모름
```

### 감사 테스트 방법 선택 (변경 관리 맥락)

| 문제 키워드 | 묻는 것 | 정답 |
|---|---|---|
| design effectiveness | 프로세스 설계가 좋은가 | 워크스루 |
| test changes / compliance | 변경이 절차를 따랐나 | 역추적 |
| operating effectiveness | 규정 준수 중인가 | 샘플 테스트 |

---

## S4. 한글 번역 함정 + 감사인 역할 구분

### 번역 함정

| 영어 | 정확한 번역 | 시험에 나올 수 있는 이상한 번역 |
|---|---|---|
...
```
"~한다" vs "~되는지 확인한다"
  "패치를 승인한다"        → 직접 행동 → 감사인 역할 ❌
  "패치가 승인되는지 확인한다" → 확인 → 감사인 역할 ✅

"IS auditor should ~" 문제에서:
  ❌ 패치를 적용/테스트/승인 (Admin/개발/운영위 역할)
  ✅ 변경 관리 프로세스에 통제가 있는지 확인

원칙: 감사인은 만드는 사람이 아니라 확인하는 사람
```
...
→ 시스템 종류마다 CIA 우선순위가 다름
→ 그 우선순위를 위협하는 게 GREATEST concern
```

> CIA + Accountability 매칭표 (통제 카테고리)는 [D4-9](#cia--accountability-매칭표-) 참고.

### 예시 1: 의료 기록 전사 서비스

```
1. 무엇? 환자 정보 다룸
2. 핵심? 기밀성(C) — 환자 데이터 보호
3. 위협? 제3자 보관 + 연 1회만 재고 확인 → GREATEST
```

### 예시 2: 해외 미러링 DB (대중 공개)

```
1. 무엇? 대중에게 빠르게 정보 제공
2. 핵심? 가용성(A) — 빠른 응답이 목적
   ❌ 기밀성 (공개 정보라 무관)
3. 위협? 해외 하드웨어 고장 (수리 어려움) → GREATEST

함정:
  ❌ 기밀성 (DB 보면 본능적으로 끌리지만 공개 정보)
  ❌ 백업 (미러본이 본사에 있음)
  ❌ 원격 접근 (가용성과 무관)
```
...
문제에서 단어 하나하나가 단서:
  ...
  "general public/공개"   → 기밀성 무관
  "speed/timely/빠른"     → 가용성 핵심
  "overseas/해외"        → 하드웨어 대응 어려움
  "mirror/미러링"        → 백업 걱정 X
  "patient/환자"         → 기밀성 최우선
  "transaction/거래"     → 무결성 핵심
  "cloud/제3자"          → 데이터 접근/위치 이슈
```

### ⚠️ 실무 vs 시험 마인드

```
요즘 실무: 클라우드 쓰면 하드웨어 신경 X
CISA 시험: 전통 IT 환경 가정 (자체 데이터센터, 물리 서버)

문제 키워드별 가정:
  "data center / overseas / physical" → 자체 인프라
  "cloud / SaaS / third-party"        → 클라우드

실무 경험으로 "클라우드면 되지" 본능 → 함정
시험 볼 때는 "옛날 방식 IT"로 가정
```
...
2. 준수 테스트 (Compliance)
       ↓
3. 실증 테스트 (Substantive)
```

### 왜 이 순서인가
```
통제 작동 → 데이터 신뢰 → 실증은 샘플만 OK
통제 결함 → 데이터 신뢰 불가 → 전수 검사 필요

비유: 공장 품질 검사 작동(준수) → OK면 샘플(실증) / 안 되면 전수
```

```
"준수 테스트 실패하면?" → 실증 X, 통제 결함 보고 또는 재평가
```
...
| 유형 | 시점 | 목적 | 예시 |
|---|---|---|---|
| **Preventive** (예방) | 사전 | 발생 자체 막기 | 접근 제어, 직무분리, 입력 검증, 암호화, 무결성 제약, Limit Check |
| **Detective** (탐지) | 사후 | 발생 후 발견 | 로그 검토, 모니터링, 해시 비교, 감사 추적 |
| **Corrective** (시정/복구) | 사후 | 발생 후 복구/수정 | **복원, 롤백(Back-out), 백업 복구, 패치, 사고 대응, 재해 복구 사이트** |
| **Compensating** (보상) | 사전/사후 | 주 통제 보완 | 주 통제 어려울 때 대안 (DBA 야간 약식 + 로그 + 다음날 검토 등) |

```
키워드 → 통제 유형:
  prevent/avoid → Preventive
  detect/find/identify → Detective
  corrective/restore/recover → Corrective ★
  compensating → Compensating

문제가 통제 유형을 명시하면 → 그 유형만 골라야 함
  "corrective control 추천" → 예방/탐지 선택지 자동 탈락
```
...
"FIRST" 키워드 보이면:
  ...
  ✅ 거의 정답: 확인 / 이해 / 검토 / 파악 / 인터뷰
  ❌ 거의 함정: 보고 / 권고 / 작성 / 변경 / 만들기

이 한 줄만 기억하면 FIRST 문제는 거의 다 풀림
```
...
A: 감사 연기 ❌ (회피)
B: 경영진에 보고 ❌ (시기상조 — 평가도 안 함)
C: 양 부서에 합의 내용 확인 ✅ (이해 단계)
D: SLA 초안 작성 ❌ (감사인 역할 X)
...
CISA 정답: 일단 더 파악 ✅
  → 패턴 8 (원인 파악 먼저)와 동일
  → 평가도 안 한 상태에서 보고 = 시기상조
```

### ⚠️ 예외: 긴급 보안 사고는 즉시 조치

```
일반 감사 발견: 이해 → 평가 → 권고 → 보고
긴급 보안 사고: 즉시 봉쇄 → 조사 → 보고

구분 키워드:
  "discovered/observed/found" → 일반 발견 → 이해 먼저
  "stolen/breached/compromised" → 보안 사고 → 즉시 조치 ★

예시: DBA 노트북 도난 + 운영 DB 비밀번호 파일
  ❌ 감사부서 보고 (느림)
  ❌ DBA 계정명 변경 (운영 영향)
  ❌ DBA 계정 정지 (업무 마비)
  ✅ DB 비밀번호 즉시 변경 (Containment)

인시던트 대응 5단계:
  1. Identification (식별)
  2. Containment (봉쇄) ★ FIRST
  3. Eradication (근절)
  4. Recovery (복구)
  5. Lessons Learned (사후 분석/보고)
```
...
> 무단 변경 3가지 패턴(준수/해시/역추적)은 [S3](#s3-무단-변경-관련-3가지-패턴-) 참고.
...
| **Walk-through** | ① 문서 현행화 ② 계획 간(BCP-IR) 정합성 검증 | "문서 점검/교차 확인" |
...
> OS Hardening 일반은 [D5-2](#d5-2-서버os-hardening) 참고.
...
> DB Hardening / 초기화 파라미터는 [D4-9](#db-하드닝-hardening--초기화-파라미터) 참고.
...
> detect vs prevent 동사 구분은 [S2](#s2-prevent-vs-detect-통제-구분-) 참고.
